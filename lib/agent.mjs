...
// Claude Opus 5, USD per million tokens. Shown in the UI so a session of
// regenerating has a visible price.
...
function getClient() {
  if (!process.env.ANTHROPIC_API_KEY) {
    // The hosted board runs keyless on purpose: it serves the committed
    // library, and authoring happens on a machine that has a key. So this is
    // the expected answer there, not a misconfiguration.
    throw new HttpError(503, 'Generation is off: no ANTHROPIC_API_KEY. '
      + 'Cues are written locally (copy .env.example to .env and add a key) and land here through a PR.');
  }
...
function usage(message, model) {
  ...
  return {
    ...
    input,
    output,
    costUsd: Number(((input / 1e6) * PRICE.in + (output / 1e6) * PRICE.out).toFixed(4)),
  };
}
